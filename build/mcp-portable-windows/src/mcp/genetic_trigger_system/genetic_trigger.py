...
class GeneticTrigger:
    ...
    @staticmethod
    def encode_dict_to_codon(data: Any) -> str:
        """
        Encode data to a 256-codon DNA string using 4-letter codons.
        """
        import json
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if hasattr(value, '__dict__') or hasattr(value, 'to_dict'):
                    serializable_data[key] = convert_to_dict(value)
                else:
                    serializable_data[key] = value
        else:
            # Convert non-dict objects to dict format
            serializable_data = convert_to_dict(data)
        
        b = json.dumps(serializable_data, sort_keys=True).encode('utf-8')
    ...
    def __init__(self, 
        ...
        Args:
            ...
            genetic_sequence: DNA sequence representing the genetic configuration.
            activation_threshold: Threshold for activation (0.0 to 1.0).
        """
        self.id = trigger_id
        ...
        self.pathway_config = {"sequence": genetic_sequence, "threshold": activation_threshold}
    ...
    async def should_activate(self, environment: Dict[str, Any], threshold: float = 0.7, resource_metrics: Optional[Dict[str, float]] = None) -> bool:
        """
    ...
    def mutate(self) -> 'GeneticTrigger':
        ...
        """
        # Convert EnvironmentalState to dict if needed
        if hasattr(self.formation_environment, '__dict__'):
            env_dict = self.formation_environment.__dict__
        elif hasattr(self.formation_environment, 'to_dict'):
            env_dict = self.formation_environment.to_dict()
        elif isinstance(self.formation_environment, dict):
            env_dict = self.formation_environment
        else:
            env_dict = dict(self.formation_environment)
        
        # Create a copy of the formation environment
        mutated_environment = dict(env_dict)
        ...
        mutated_trigger_id = f"mutated_{self.id}_{int(datetime.now().timestamp())}"
        mutated_trigger = GeneticTrigger(
            trigger_id=mutated_trigger_id,
            formation_environment=mutated_environment,
            genetic_sequence=self.genetic_sequence,
            activation_threshold=self.activation_threshold
        )
        mutated_trigger.parent_ids = [self.id]
    ...
    def crossover(cls, trigger1: 'GeneticTrigger', trigger2: 'GeneticTrigger') -> 'GeneticTrigger':
        ...
        """
        # Convert EnvironmentalState objects to dicts if needed
        ...
        env1_dict = get_env_dict(trigger1.formation_environment)
        env2_dict = get_env_dict(trigger2.formation_environment)
        
        # Combine environment dictionaries
        combined_env = {}
        all_keys = set(env1_dict.keys()) | set(env2_dict.keys())
        
        for key in all_keys:
            # Choose value from either parent
            if key in env1_dict and key in env2_dict:
                # Both parents have this key, randomly choose or blend
                if random.random() < 0.5:
                    combined_env[key] = env1_dict[key]
                else:
                    combined_env[key] = env2_dict[key]
            elif key in env1_dict:
        ...
        # Create a new genetic sequence by combining parent sequences
        combined_sequence = trigger1.genetic_sequence[:len(trigger1.genetic_sequence)//2] + trigger2.genetic_sequence[len(trigger2.genetic_sequence)//2:]
        
        # Create a new trigger with the combined values
        child_trigger_id = f"crossover_{trigger1.id}_{trigger2.id}_{int(datetime.now().timestamp())}"
        child_trigger = cls(
            trigger_id=child_trigger_id,
            formation_environment=combined_env,
            genetic_sequence=combined_sequence,
            activation_threshold=0.7
        )
        child_trigger.parent_ids = [trigger1.id, trigger2.id]
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'GeneticTrigger':
        ...
        """
        # Extract genetic sequence from pathway config or use default
        genetic_sequence = data["pathway_config"].get("sequence", "ATGCGATCGTAGC")
        activation_threshold = data["pathway_config"].get("threshold", 0.7)
    ...
    def multi_point_crossover(parent1: 'GeneticTrigger', parent2: 'GeneticTrigger', points: int = 2) -> 'GeneticTrigger':
        ...
        if not env1 and not env2:
            ...
            child_trigger_id = f"multicross_{parent1.id}_{parent2.id}_{int(datetime.now().timestamp())}"
            return GeneticTrigger(child_trigger_id, {}, "ATGCGATCGTAGC", 0.7)
        ...
        if length < 2:
            ...
            # Fill in any missing keys from longer parent
            for k, v in (env1 + env2):
                if k not in child_env:
                    child_env[k] = v
        # Create combined genetic sequence
        combined_sequence = parent1.genetic_sequence[:len(parent1.genetic_sequence)//2] + parent2.genetic_sequence[len(parent2.genetic_sequence)//2:]
        
        # Create child trigger
        child_trigger_id = f"multicross_{parent1.id}_{parent2.id}_{int(datetime.now().timestamp())}"
        child_trigger = GeneticTrigger(child_trigger_id, child_env, combined_sequence, 0.7)
        child_trigger.parent_ids = [parent1.id, parent2.id]
        return child_trigger
