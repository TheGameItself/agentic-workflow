...
def eliminate_stubs_in_project(project_root: Optional[str] = None, dry_run: bool = True) -> Dict[str, Any]:
    """Convenience function to eliminate stubs in a project."""
    engine = StubEliminationEngine(project_root)
    stubs = engine.scan_for_stubs()
    replacements = engine.generate_replacements(stubs)
    results = engine.apply_replacements(replacements, dry_run=dry_run)
    return {
        'scan_results': engine.get_stub_report(),
