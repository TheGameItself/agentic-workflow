...
def main():
    ...
    def _get_python_interpreter_path(self) -> Optional[str]:
        """Get the appropriate Python interpreter path."""
        venv_paths = [
            self.project_root / "venv" / ("bin" if self.system != "windows" else "Scripts") / ("python" if self.system != "windows" else "python.exe"),
            self.project_root / ".venv" / ("bin" if self.system != "windows" else "Scripts") / ("python" if self.system != "windows" else "python.exe"),
        ]
        ...
        # Fallback to system Python
        try:
            cmd = ["which", "python3"] if self.system != "windows" else ["where", "python"]
...
'''
    ...
    def create_portable_vscode_package(self, output_dir: str = "portable_vscode_mcp"):
        ...
        try:
            ...
            steps = [
                lambda: self._copy_project_files(output_path),
                lambda: self._setup_portable_vscode_config(output_path),
                lambda: self._create_portable_launcher(output_path)
            ]
