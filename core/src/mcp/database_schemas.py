...
# Database paths
DB_PATHS = {
    "hormone": "data/hormone_system.db",
    "genetic": "data/genetic_trigger_system.db",
    "monitoring": "data/monitoring_system.db"
}

# Schema mapping
SCHEMAS = {
    "hormone": HORMONE_SYSTEM_SCHEMA,
    "genetic": GENETIC_TRIGGER_SCHEMA,
    "monitoring": MONITORING_SCHEMA
}


def execute_sql(db_path: str, sql: str, params=None) -> sqlite3.Cursor:
...
def initialize_all_databases() -> None:
    ...
    for db_type, db_path in DB_PATHS.items():
        initialize_database(db_path, SCHEMAS[db_type])
...
if __name__ == "__main__":
    ...
    for db_type, db_path in DB_PATHS.items():
        version = check_schema_version(db_path)
        print(f"{db_type.capitalize()} system schema version: {version}")
